...
export async function processAudioGeneration(generatedInfo, preferences, testMode, iosSafari, openAiApiKey) {
  ...
  console.log(`Text processing complete:`, {
    originalLength: generatedInfo.length,
    processedLength: textForAudio.length,
    truncated: generatedInfo.length !== textForAudio.length,
    testMode: testMode,
    safariOptimized: isSafariOptimized
  });
